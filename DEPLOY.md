...
- [ ] 域名已解析到服务器公网 IP,且**已完成 ICP 备案**(腾讯云对绑定域名的 80/443 访问有备案要求,不备案很多场景会被拦)
- [ ] `backend/.env` 在**服务器上**单独配置(不是从本地复制开发用的那份):
  - `NODE_ENV=production`
  - `BACKEND_URL` / `PUBLIC_URL` = 真实后端域名(如 `https://api.你的域名.com`)
  - `FRONTEND_URL` 或 `CORS_ORIGINS` = 真实前端域名,**去掉所有 localhost**
...
- [ ] `frontend/.env`(或构建时环境变量)的 `VITE_API_URL` = 真实后端域名——**这个是打包时写死进静态文件的,构建前必须改对,改完要重新 build**
- [ ] 微信支付回调地址、腾讯云短信签名的报备信息与实际部署域名一致
...
```bash
cd /var/www   # 或你习惯的目录
git clone https://github.com/<你的仓库>/docflow-ai.git
cd docflow-ai
```

私有仓库在服务器上拉取需要认证:用 GitHub 个人访问令牌(`git clone https://<token>@github.com/...`),或给服务器配一把只读 Deploy Key。
...
```bash
cd ../frontend
echo "VITE_API_URL=https://api.你的域名.com" > .env.production.local
npm install
...
- [ ] `curl https://api.你的域名.com/health` 返回正常
